function [U,cur_sol,nn,Lb,status]=BB_QBL_Heur_EBB(A,b,Q0,b0,b2,c2,N,gap)
...
[x_trs,yloc,mult,~,~,~,mu_glo]=TRS_Loc(Q0,b0,b2,-0.5*c2);
f_trs=0.5*x_trs(:,1)'*Q0*x_trs(:,1)-b0'*x_trs(:,1);
load chinexa
if rrr==1 && mult>1
[yy pp]=hardoptfind(x_trs,eye(size(A,2)),zeros(size(A,2),1),mult,mu_glo,Q0,b0...
                  ,zeros(size(A,2),1),sqrt(-c2),30);
else 
    yy=5000*rand(size(A,2),1);
end
  if  max(A*yy-b)<1e-5 && rrr==1%rrr==13%
   U=f_trs;
   cur_sol=yy;
   nn=0;
   Lb=f_trs;
